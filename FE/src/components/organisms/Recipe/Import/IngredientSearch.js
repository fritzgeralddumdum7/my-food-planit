...
import { Group, Text, Box, TextInput, Loader, Button } from '@mantine/core';
...
const IngredientSearch = ({ ingredient, index }) => {
  ...
  const searchRecipe = (values) => {
    setLoading(true);
    const parsed = parse(values.ingredient);
    ...
    API.request({
      ...
      params: {
        ingrs: [
          `${parsed.quantity ?? ''} ${parsed.unit ?? ''} ${parsed.ingredient}`,
        ],
      },
    }).then(({ data }) => {
              ...
              <Group sx={{ cursor: 'pointer', width: 85 }}>
                {hovered && (
                  <Box
                    ...
                    onClick={() => deleteRecipeIngredients(index, false)}
                  >
                    <Trash />
