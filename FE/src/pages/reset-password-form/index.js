...
const Index = () => {
    ...
    validate: {
      password: (value) => {
        if (value === '') {
          ...
          return 'Your password must be between 8 to 24 characters, and should have at least 1 uppercase letter, 1 lowercase letter, and 1 special character.';
        }
        return null;
      },
  ...
  //   }
  //   const params = new URLSearchParams(window.location.search);
  //   // Manually get the query params instead of using next/router
  //   console.log({
  //     token: params.get('token'),
  //     email: params.get('email'),
  //   });
  //   form.setValues({ token: params.get('token'), email: params.get('email') });
  // }, [router.isReady]);
