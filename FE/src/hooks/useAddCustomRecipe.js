...
const useAddCustomRecipe = () => {
  ...
  const handleAddIngrs = async (values) => {
    modalDispatch({
      type: 'showSpinner',
      payload: {
    ...
    const callback = (result) => {
      const ingrs = parseIngredients(JSON.parse(result.data.nutritions));
      const newIngrsList = [
        ...(recipe.ingrWithNutr ? recipe.ingrWithNutr.match : []),
        ...ingrs.match,
      ];
      setRecipe({
        ...recipe,
  ...
  return {
    ...
    addCustomRecipeHandleAddIngrs: handleAddIngrs,
    addCustomRecipeDeleteIngr: deleteIngredient,
  };
};

export default useAddCustomRecipe;
