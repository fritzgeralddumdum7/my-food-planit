...
const CircularProgressBar = ({ limit, progress }) => {
  const data = {
    ...
    datasets: [
      {
        data: [progress >= limit ? 0 : parseInt(limit - progress), parseInt(progress)],
        backgroundColor: function (context) {
          ...
          return ['#DEE1E6', getGradient(ctx, chartArea)];
        },
        borderWidth: 0,
      },
    ],
  ...
  const remainingCal = () => {
    const remain = parseInt(limit - progress);
