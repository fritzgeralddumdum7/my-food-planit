...
const Index = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const stepOneClear = !formData.errors.hasOwnProperty('mfpHelp') && id == 1;
    if (stepOneClear) {
      const step = formData.values.mfpHelp ? 2 : 7;
      router.push(
        `/register/onboarding/${step}?mfpHelp=${formData.values.mfpHelp}`
      );
    }
    ...
    if (mfpHelp == 'true' || mfpHelp == 'null') {
      ...
      if (!formData.errors.hasOwnProperty('nineToTenMode') && id == 6) {
        ...
        router.push(
          `/register/onboarding/7?mfpHelp=${formData.values.mfpHelp}`
        );
      }
    } else {
      if (!formData.errors.hasOwnProperty('mfpHelp') && id == 1) {
        Cookies.set('onboarding', JSON.stringify(formData.values));
  ...
  return (
    <Main meta={<Meta title='My Food Plan It' description='Lorem Ipsum' />}>
      ...
      >
        ...
        90/10 Nutrition is a system of clean eating created by the very same people that created MyFoodPlanIt!
        <br />
        90/10 is based on three simple tiers. You eat 90% of your daily intake from the green tier (the cleanest foods), no more than 10% from the yellow tier, and nothing from the red tier. It’s that simple. You can see the tiers below.
        <br />
        <br />
        You can choose to turn on “90/10 mode” in MyFoodPlanIt and the system will help you track based on the tiers rather than calories and macronutrients. It’s great for those that want to eat the 90/10 way. Give it a try!
        <br />
        <br />
